 ...
 */
function getStructuredDataFromMeta(): Record<string, any> {
 ...
 */
function getAllStructuredData(): Record<string, any> {
  const result: Record<string, any> = {
  ...
  function isLikelyStructuredData(obj: any, key: string): boolean {
  ...
  for (const [key, value] of Object.entries(hydrationData)) {
    if (key === 'structuredData') continue; // Skip the meta data we already have
  ...
  const dataAttributeData: Record<string, any> = {};
  ...
  allElements.forEach(element => {
    ...
    if (dataAttributes.length === 0) return; // Skip elements with no data attributes
  ...
  if (typeof (window as any).__NEXT_DATA__ === 'object') {
    result.nextData = (window as any).__NEXT_DATA__;
  }
  
  // Extract data from window.__NUXT__ (Nuxt.js)
  if (typeof (window as any).__NUXT__ === 'object') {
    result.nuxtData = (window as any).__NUXT__;
  }
  
  // Extract data from window.__INITIAL_STATE__ (common in Redux apps)
  if (typeof (window as any).__INITIAL_STATE__ === 'object') {
    result.initialState = (window as any).__INITIAL_STATE__;
  }
  
  // Extract data from window.__PRELOADED_STATE__ (common in Redux apps)
  if (typeof (window as any).__PRELOADED_STATE__ === 'object') {
    result.preloadedState = (window as any).__PRELOADED_STATE__;
  }
  
  // Extract data from window.__APOLLO_STATE__ (Apollo GraphQL)
  if (typeof (window as any).__APOLLO_STATE__ === 'object') {
    result.apolloState = (window as any).__APOLLO_STATE__;
  }
  
  // Extract data from window.REDUX_STATE (another Redux pattern)
  if (typeof (window as any).REDUX_STATE === 'object') {
    result.reduxState = (window as any).REDUX_STATE;
  }
  
  // Extract data from window.APP_INITIAL_STATE (common pattern)
  if (typeof (window as any).APP_INITIAL_STATE === 'object') {
    result.appInitialState = (window as any).APP_INITIAL_STATE;
  }
  
  // Extract RDFa data
  const rdfaData: any[] = [];
  document.querySelectorAll('[typeof], [property]').forEach(element => {
    const rdfaItem: Record<string, any> = {};
...
(window as any).searchHydration = searchAllData;
(window as any).searchAllData = searchAllData;
(window as any).getStructuredDataFromMeta = getStructuredDataFromMeta;
(window as any).getAllStructuredData = getAllStructuredData;
